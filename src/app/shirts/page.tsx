...
export default function ShirtsPage() {
  ...
  useEffect(() => {
    async function fetchShirts() {
      setLoading(true);
      // Fetch all, then filter in JS to avoid case-sensitivity issues from Supabase
      const { data, error } = await supabase.from('products').select('*');

      if (!error && data) {
        const onlyShirts = data.filter(p => 
          p.category?.trim().toLowerCase() === 'shirts'
        );
        setProducts(onlyShirts);
      }
      setLoading(false);
    }
    fetchShirts();
    ...
    <VibePage>
      <div className="max-w-7xl mx-auto px-6 py-20">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-serif italic tracking-tighter uppercase text-black">Shirts</h1>
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em] mt-2 italic">Essential Silhouette</p>
        </div>
        ...
        {!loading && products.length === 0 && (
          <p className="text-center text-gray-300 uppercase text-[10px] tracking-widest py-20 italic">
            The shirt vault is currently empty.
          </p>
        )}
      </div>
    </VibePage>
